  ...
  ({ falcor, falcorCache, ...props }) => {
    ...
    React.useEffect(() => {
      ...
      falcor.get([
        ...
        falcor.get(["tds", "short", "class", "weeklyAvg", "byCountId", metaId, "array"])
      ).then(() =>
        falcor.get([...basePath, metaId, "length"])
      ).then(res => {
    ...
    const weeklyAvg = React.useMemo(() => {
      return get(falcorCache, ["tds", "short", "class", "weeklyAvg", "byCountId", metaId, "array", "value"], []);
    }, [metaId, falcorCache]);
